...
async function main() {
  ...
  try {
    ...
    console.log("");
    console.log(
      "2. Use the following command in Discord #devnet-faucet channel:"
    );
    console.log(`   !faucet ${adminAddress}`);
    console.log("");
    console.log("3. Or use the REST API:");
    console.log(
      `   curl -X POST https://faucet.devnet.sui.io/gas -H "Content-Type: application/json" -d '{"FixedAmountRequest":{"recipient":"${adminAddress}"}}'`
    );
    console.log("");
    console.log("🔍 Check balance at:");
    console.log(
      `   https://suiexplorer.com/address/${adminAddress}?network=devnet`
    );
    console.log("");
