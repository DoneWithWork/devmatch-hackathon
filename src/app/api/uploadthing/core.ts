...
const auth = async () => {
  ...
  const user = await db.query.users.findFirst({
    where: eq(users.id, session.id), // Remove the + conversion for UUID
  });
  console.log(user);
  return user;
}; // Fake auth function
  ...
  imageUploader: f({
    ...
    .onUploadComplete(async ({ metadata, file }) => {
      ...
      const newIssuerDocument = await db.insert(issuerDocuments).values({
        ...
        fileName: file.name, // Add required fileName field
        userId: metadata.userId,
      });
      if (!newIssuerDocument)
        throw new Error("Failed to create new issuer document");
