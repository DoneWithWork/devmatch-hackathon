...
export async function getSession(cookies: ReadonlyRequestCookies) {
    const session = await getIronSession<SessionData>(cookies, { password: env.SESSION_KEY, cookieName: "hashcred" })
    return session
}

type SaveSessionProps = SessionData & { cookies: ReadonlyRequestCookies }
export async function SaveSession({ cookies, id, maxEpoch, randomness, userAddress, role }: SaveSessionProps) {
